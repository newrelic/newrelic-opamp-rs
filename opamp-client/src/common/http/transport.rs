...
pub struct HttpTransport<C, T = ReqwestSender>
where
    C: Callbacks,
    T: Transport,
{
...
impl<C: Callbacks, T: Transport> HttpTransport<C, T> {
    ...
    // next_send returns the unit type if a new message should be sent, none if notifying
    ...
    async fn receive(
        ...
        if msg
            ...
            .map(|c| self.callbacks.on_command(c))
            .is_some()
        {
            return None;
        }
    ...
    T: Transport + Send + Sync,
{
    ...
    async fn run(
        ...
        info!("HTTP Forwarder Receiving channel closed! Exiting");
        Ok(())
    }
}
...
pub(crate) mod test {
    ...
    async fn test_http_client_run_receive_message_on_command() {
        ...
        transport
            .expect_send()
            .once()
            .returning(move |_| Ok(reqwest_response_from_server_to_agent(&server_to_agent)));
    ...
    fn reqwest_response_from_server_to_agent(server_to_agent: &ServerToAgent) -> Response {
        ...
        let response = Builder::new().status(200).body(buf).unwrap();
